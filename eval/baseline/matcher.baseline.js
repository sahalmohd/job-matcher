const JobMatcher = (() => {
  ...
  function scoreJob(resumeText, job, weights = DEFAULT_WEIGHTS) {
    ...
    // TF-IDF + keyword overlap similarity (0-100)
    ...
    // When the job text is too short to extract any skills, rely entirely on
    // the text-similarity score instead of averaging in a guaranteed 0.
    ...
    const finalScore =
      Math.round((tfidfScore * effectiveWeights.tfidf + skillScore * effectiveWeights.skills) * 100) / 100;
