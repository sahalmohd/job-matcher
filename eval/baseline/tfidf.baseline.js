const TFIDF = (() => {
  ...
  /**
   * Simple keyword overlap ratio — what fraction of job tokens appear in the resume.
   * More robust than TF-IDF for very short texts.
   */
  function keywordOverlap(resumeTokens, jobTokens) {
    if (jobTokens.length === 0) return 0;
    const resumeSet = new Set(resumeTokens);
    let hits = 0;
    for (const t of jobTokens) {
      if (resumeSet.has(t)) hits++;
    }
   ...
   * Uses TF-IDF cosine similarity for normal-length text, and blends in
   * keyword overlap for short job text where TF-IDF is unreliable.
  ...
  function score(resumeText, jobText) {
    ...
    const cosSim = cosineSimilarity(resumeVec, jobVec);
    const overlap = keywordOverlap(resumeTokens, jobTokens);

    // Short job text (< 20 unique tokens) → lean on keyword overlap;
    // longer text → lean on TF-IDF cosine similarity.
    const shortness = Math.max(0, Math.min(1, 1 - (jobTokens.length - 5) / 30));
    const blended = cosSim * (1 - shortness) + overlap * shortness;
